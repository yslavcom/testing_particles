...
void enumAudioDevices(int index, const char** names)
{
	if (NULL == names)return;
	
	const char* new_name = SDL_GetAudioDeviceName(index, 0);
	size_t size = strlen(new_name) + 1;
	char* p = malloc(size);
	if (NULL != p)
	{
		*names = p;
		memcpy(p, new_name, size);
	}
	
}

void initAudio(const char * device)
{
	...
	/* want.userdata = new; */
	if ((gDevice->device = SDL_OpenAudioDevice(gDevice->device_name, 0, &(gDevice->want), NULL, SDL_AUDIO_ALLOW_CHANGES)) == 0)
	{
...
int main()
{
	...
	int count = getAudioDevicesCount();
	const char** names = new const char* [count];
	for (int i = 0; i < count; i++)
	{
		enumAudioDevices(i, &names[i]);
		std::cout << names[i] << std::endl;
	}

	/* Init Simple-SDL2-Audio */

	const int headphones = 0;
	const int monitor = 1;
	const int audio_device = headphones;
	initAudio(names[audio_device]);


	/* Play music and a sound */
	//playMusic("music/highlands.wav", SDL_MIX_MAXVOLUME/8);
	//playMusic("music/road.wav", SDL_MIX_MAXVOLUME / 8);
	Audio* music = createAudio("music/highlands.wav", 1, SDL_MIX_MAXVOLUME / 8);
	...
	for (;;);
	SDL_Delay(5000);
	std::cout << "delay is over" << std::endl;
